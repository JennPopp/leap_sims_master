   ...
   if(versionType=="Pol"){
    ...
    //G4cout<< " This part of the code you are currently testing is executed"  << G4endl;
       ...
       if ( postvolume == VacStep2PV && prevolume !=VacStep2PV ) {
           ...
           fAnalysisManager->FillNtupleDColumn(2,1, aStep->GetPostStepPoint()->GetKineticEnergy()/MeV);

           fAnalysisManager->FillNtupleDColumn(2,2, aStep->GetPostStepPoint()->GetPosition().x());
           fAnalysisManager->FillNtupleDColumn(2,3, aStep->GetPostStepPoint()->GetPosition().y());
           fAnalysisManager->FillNtupleDColumn(2,4, aStep->GetPostStepPoint()->GetPosition().z());

           fAnalysisManager->FillNtupleDColumn(2,5, aStep->GetTrack()->GetVertexPosition().x());
           fAnalysisManager->FillNtupleDColumn(2,6, aStep->GetTrack()->GetVertexPosition().y());
           fAnalysisManager->FillNtupleDColumn(2,7, aStep->GetTrack()->GetVertexPosition().z());

           fAnalysisManager->FillNtupleDColumn(2,8, aStep->GetPostStepPoint()->GetMomentumDirection().x());
           fAnalysisManager->FillNtupleDColumn(2,9, aStep->GetPostStepPoint()->GetMomentumDirection().y());
           fAnalysisManager->FillNtupleDColumn(2,10, aStep->GetPostStepPoint()->GetMomentumDirection().z());

           fAnalysisManager->FillNtupleDColumn(2,11, aStep->GetTrack()->GetPolarization().x());
           fAnalysisManager->FillNtupleDColumn(2,12, aStep->GetTrack()->GetPolarization().y());
           fAnalysisManager->FillNtupleDColumn(2,13, aStep->GetTrack()->GetPolarization().z());

           fAnalysisManager->FillNtupleDColumn(2,14, aStep->GetTrack()->GetTrackID());
           fAnalysisManager->FillNtupleDColumn(2,15, aStep->GetTrack()->GetParentID());
           fAnalysisManager->FillNtupleDColumn(2,16, G4RunManager::GetRunManager()->GetCurrentEvent()->GetEventID());
      ...
      if ( postvolume == VacStep3PV && prevolume !=VacStep3PV && aTrack->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()) {
          ...
          fAnalysisManager->FillNtupleDColumn(1,1,aStep->GetPostStepPoint()->GetTotalEnergy()/eV );
          fAnalysisManager->FillNtupleDColumn(1,2,theTouchable->GetReplicaNumber(2));  // here the 1 means that it takes the copy numer of its mother volume
      ...
      if( postvolume == VacStep4PV && prevolume !=VacStep4PV && prevolume != AluwrapPV && aTrack->GetParticleDefinition()->GetPDGEncoding() == 22) {
          ...
          fAnalysisManager->FillNtupleDColumn(2,2,theTouchable->GetReplicaNumber(1));  // here the 1 means that it takes the copy numer of its mother volume
      ...
      //G4cout<< " This part of the code you are currently testing is executed"  << G4endl;
         ...
         if ( postvolume == VacStep2PV && prevolume !=VacStep2PV ) {
          ...
          if ( postvolume == VacStep3PV && prevolume !=VacStep3PV && aTrack->GetDefinition() == G4OpticalPhoton::OpticalPhotonDefinition()) {
              ...
              fAnalysisManager->FillNtupleDColumn(3,1,aStep->GetPostStepPoint()->GetTotalEnergy()/eV );
              fAnalysisManager->FillNtupleDColumn(3,2,theTouchable->GetReplicaNumber(2));  // here the 1 means that it takes the copy numer of its mother volume
            ...
            if( postvolume == VacStep4PV && prevolume !=VacStep4PV && prevolume != AluwrapPV && aTrack->GetParticleDefinition()->GetPDGEncoding() == 22) {
              ...
              // fill ntuple id=3
              fAnalysisManager->FillNtupleIColumn(4,0, aStep->GetTrack()->GetParticleDefinition()->GetPDGEncoding());

              fAnalysisManager->FillNtupleDColumn(4,1,aStep->GetPostStepPoint()->GetTotalEnergy()/MeV );
              fAnalysisManager->FillNtupleDColumn(4,2,theTouchable->GetReplicaNumber(1));  // here the 1 means that it takes the copy numer of its mother volume

              fAnalysisManager->FillNtupleDColumn(4,3, aStep->GetPostStepPoint()->GetPosition().x()/mm);
              fAnalysisManager->FillNtupleDColumn(4,4, aStep->GetPostStepPoint()->GetPosition().y()/mm);
              fAnalysisManager->FillNtupleDColumn(4,5, aStep->GetPostStepPoint()->GetPosition().z()/mm);
